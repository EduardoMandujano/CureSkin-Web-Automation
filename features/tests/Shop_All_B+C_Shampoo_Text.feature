# Created by Eduardo Mandujano at 8/12/2023
Feature: Shop All Main Page B+C Shampoo Text Verification
  Verify the correct navigation of Shop All Page using text

  Scenario: Shop All Main Page B+C Shampoo Text Verification
    Given Open CureSkin Home Page
    # When Click on the X to close the Popup Window
    When Click on the Shop All Header Link
    And Click on the BC Shampoo Link
    Then Verify the text of the BC Shampoo in the Product Details Page